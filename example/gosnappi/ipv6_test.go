...
// PORT1=10.109.121.181/1/1 PORT2=10.109.123.254/1/1 OTGSERVER=localhost:50051 go test -v -test.run TestIpv6
...
func TestIpv6(t *testing.T) {
	...
	//ipv6.Dst().SetValues([]string{"2000::1", "2000::2", "2000::3", "2000::4"})
	//ipv6.Dst().Increment().SetStart("2000::1").SetStep("::1").SetCount(100)
	//ipv6.Dst().Decrement().SetStart("2000::64").SetStep("::1").SetCount(100)
	ipv6.Src().SetValue("2001::1")
	//ipv6.Src().SetValues([]string{"2001::1", "2001::2", "2001::3", "2001::4"})
	//ipv6.Src().Decrement().SetStart("2001::64").SetStep("::1").SetCount(100)
	//ipv6.Src().Increment().SetStart("2001::1").SetStep("::1").SetCount(100)

	//ipv6.FlowLabel().SetValue(1048575)
	//ipv6.FlowLabel().SetValues([]uint32{15, 255, 4095, 65535})
	//ipv6.FlowLabel().Increment().SetStart(15).SetStep(1).SetCount(100)
	//ipv6.FlowLabel().Decrement().SetStart(65535).SetStep(2).SetCount(100)

	//ipv6.HopLimit().SetValue(64)
	//ipv6.HopLimit().SetValues([]uint32{0, 10, 20, 64, 255})
	//ipv6.HopLimit().Increment().SetStart(1).SetStep(1).SetCount(254)
	//ipv6.HopLimit().Decrement().SetStart(255).SetStep(1).SetCount(254)
	//ipv6.PayloadLength().SetValue(1)
	//ipv6.PayloadLength().SetValues([]uint32{0, 100, 200, 1000, 65535})
	//ipv6.PayloadLength().Increment().SetStart(1).SetStep(8).SetCount(100)
	//ipv6.PayloadLength().Decrement().SetStart(65535).SetStep(1).SetCount(100)
	//ipv6.TrafficClass().SetValue(139)
	//ipv6.TrafficClass().SetValues([]uint32{10, 100, 139, 255})
	//ipv6.TrafficClass().Increment().SetStart(1).SetStep(1).SetCount(254)
	//ipv6.TrafficClass().Decrement().SetStart(255).SetStep(1).SetCount(254)
	//ipv6.Version().SetValue(15)
	//ipv6.Version().SetValues([]uint32{1, 2, 3, 4, 5, 6})
	//ipv6.Version().Increment().SetStart(1).SetStep(1).SetCount(14)
	//ipv6.Version().Decrement().SetStart(15).SetStep(1).SetCount(14)
	//ipv6.NextHeader().SetValue(100)
	//ipv6.NextHeader().SetValues([]uint32{10, 20, 30, 40, 100, 200, 255})
	//ipv6.NextHeader().Increment().SetStart(1).SetStep(1).SetCount(254)
	//ipv6.NextHeader().Decrement().SetStart(255).SetStep(1).SetCount(254)
