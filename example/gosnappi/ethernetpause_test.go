...
// PORT1=10.109.121.181/1/1 PORT2=10.109.123.254/1/1 OTGSERVER=localhost:50051 go test -v -test.run TestEthPause

func TestEthPause(t *testing.T) {
	...
	ethpause := pkt.Add().Ethernetpause()

	//eth.Dst().SetValue("01:80:c2:00:00:01")
	//eth.Src().SetValue("00:11:22:33:44:66")
	//eth.EtherType().SetValue(8808)

	ethpause.Src().SetValue("00:00:01:00:00:03")
	ethpause.Dst().SetValue("01:80:c2:00:00:01")
	ethpause.EtherType().SetValue(34824)
	ethpause.ControlOpCode().SetValue(4369)
	//ethpause.Time().SetValue(4369)
	//ethpause.Time().SetValues([]uint32{4369, 4370, 4371, 4372})
	//ethpause.Time().Increment().SetStart(4369).SetStep(1).SetCount(10)
	ethpause.Time().Decrement().SetStart(4369).SetStep(1).SetCount(10)
